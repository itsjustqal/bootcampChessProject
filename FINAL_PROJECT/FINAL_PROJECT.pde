...
boolean WHITE = true;
boolean BLACK = false;
boolean turn;            // indicates a players turn
boolean gameOver;
boolean wKingMoved, bKingMoved, wRookMoved1, bRookMoved1, wRookMoved2, bRookMoved2;//it is required for castling
boolean check;//if the king is in check
boolean promote; // allows the pawn to be able to promote
PImage wKing, bKing, wQueen, bQueen, wPawn, bPawn, wRook, bRook, wKnight, bKnight, wBishop, bBishop;
PImage[][] board; //2d array of the board to places the pieces on
...
void setup() {
  ...
  textSize(width/8);
  textAlign(CENTER);
  //loading in the pictures of the pieces to play with (currently it has a white background cant find transparent
  wKing = loadImage("white king.png");
  bKing = loadImage("black king.png");
  wQueen = loadImage("white queen.png");
  bQueen = loadImage("black queen.png");
  wPawn = loadImage("white pawn.png");
  bPawn = loadImage("black pawn.png");
  wRook = loadImage("white rook.png");
  bRook = loadImage("black rook.png");
  wKnight = loadImage("white horse.png");
  bKnight = loadImage("black horse.png");
  wBishop = loadImage("white bishop.jpg");
  bBishop = loadImage("1black bishop.png");
  ...
  // loads the image of pieces and places on their starting position
  startPosition();
}
void draw() {
  //everytime it starts show the board to player so they can look at the postion of the pieces
  showBoard();
  if (gameOver) {
    fill(0, 255, 0);
...
void showBoard() {
  for (int col = 0; col<8; col++)
    for (int row = 0; row<8; row++) {
      if ((col+row)%2 == 0) fill(255, 206, 158);
      else fill(209, 139, 71);
      rect(col*width/8, row*height/8, width/8, height/8);//chessboard
      if (board[row][col] != null) image(board[row][col], col*width/8, row*height/8);//piece
      if (click) {
        if (validMove(down, right, row, col, turn, board) && !incheck(down, right, row, col, turn)) {
          fill(255, 0, 0, 100);//highlight posibble moves in red
          rect(col*width/8, row*height/8, width/8, height/8);
        }
        if (row == down && col == right && board[row][col] != null) {
          fill(0, 0, 255, 100);//highlight piece in blue
          rect(col*width/8, row*height/8, width/8, height/8);
        }
      }
    }
  /* see if you are under check and if its not game over and not promoting a piece
   if it fufills all the criteria then print CHECK to signify the other player to move their king or block with something
   */
  if (check && !gameOver && !promote) {
    fill(0, 255, 0);
    text("CHECK", 0, height/2, width, height);
  }
  /*check to promote and if not game over then promote the piece according to the players
   preference by popping up the options where the players can choose which piece they want to promote to
   */
  if (promote && !gameOver) {
    stroke(2);
    fill(200, 100);
...
//check for valid move for every single piece available in chess
